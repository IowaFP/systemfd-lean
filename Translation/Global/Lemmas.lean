...
theorem lookup_append_some {G1 G2 : List Global} {e : Entry} (wf : ⊢ (G1 ++ G2)):
  ...
  case _ mths _ ih =>   -- class decl
    ...
    cases wf; case _ wftl wfhd =>
      ...
      case _ e =>
        ...
        cases wfhd
        generalize G1_def : List.map (fun x => Core.Global.openm x.fst x.snd) mths = G1 at *
        replace h2 := Core.lookup_append_some (G1 := G1) sorry h2
        cases h2
        case _ h2 => subst G1_def; sorry
        case _ h2 =>
          rcases h2 with ⟨h2, h3⟩
          simp [Core.lookup] at h3
    -- split
    -- case _ e =>
    --   subst e;
...
:= by
 ...
 cases h; simp; sorry
 simp [bind, Except.bind_eq_ok_iff] at h; sorry
...
theorem translate_SI_lookup_none {G : Surface.GlobalEnv} {G' : Intermediate.GlobalEnv} (wf : ⊢ G) :
  ⟦ G ⟧ = .ok G' ->
...
:= by
  ...
  case _ Γ ih => -- data
    ...
    apply And.intro
    apply ih wftl h1 h2
    intro v v_in_vs; sorry
  sorry
  sorry
  sorry

theorem lookup_kind_SI_some {G : Surface.GlobalEnv} {G' : Intermediate.GlobalEnv} (wf : ⊢ G) (h : ⟦ G ⟧ = .ok G') :
...
theorem kinding_SI_transfer {G : Surface.GlobalEnv} {G' : Intermediate.GlobalEnv} (wf : ⊢ G) (h : ⟦ G ⟧ = .ok G') :
  G&Δ ⊢s T : K ->  G'&Δ ⊢ T : K
| .var h1 => .var h1
| .global h1 =>.global (lookup_kind_SI_some wf h h1)
| .app h1 h2 => .app (kinding_SI_transfer wf h h1) (kinding_SI_transfer wf h h2)
| .arrow h1 h2 => .arrow (kinding_SI_transfer wf h h1) (kinding_SI_transfer wf h h2)
| .all h1 => .all (kinding_SI_transfer wf h h1)
| .eq h1 h2 => .eq (kinding_SI_transfer wf h h1) (kinding_SI_transfer wf h h2)
...
theorem Ty.data?_SI_transfer {G : Surface.GlobalEnv} {G' : Intermediate.GlobalEnv} (wf : ⊢ G) (h : ⟦ G ⟧ = .ok G') (T : Core.Ty) :
 ...
 case _ sp => simp [Intermediate.Ty.data?]; rw[sp]; simp; sorry -- apply lookup_is_data_SI_some wf h h1

theorem spine_kinding_SI_transfer {G : Surface.GlobalEnv} {G' : Intermediate.GlobalEnv} (wf : ⊢ G) (h : ⟦ G ⟧ = .ok G') (h' : (∀ T, test T -> test' T)):
  Surface.SpineKinding v x G test T ->
  Intermediate.SpineKinding v x G' test' T
| .valid h1 h2 h3 h4 h5 =>
  .valid h1
    (by intro i; have lem := h2 i;  apply kinding_SI_transfer wf h lem)
    (kinding_SI_transfer wf h h3)
    (by apply h' _ h4)
    (by intro e i; replace h5 := h5 e i; apply Ty.data?_SI_transfer wf h T.2.2.2.2.2.fst[i] h5)
...
theorem translate_SI_wf_sound {G : Surface.GlobalEnv} {G' : Intermediate.GlobalEnv} (wf : ⊢ G) :
  ...
  case _ ctors _ ih =>
    ...
    · apply Intermediate.GlobalWf.data
      · intro i y T h3; replace c3 := c3 i y T h3
        apply And.intro; sorry; apply And.intro; apply c3.1; sorry
      · apply c2
      · apply h2
    · apply ih wftl h1
  case _ ih => -- defn
    ...
    · rcases h2 with ⟨h2, h4⟩
      ...
      · apply Intermediate.GlobalWf.classDecl
        ...
        · intro i mn R T tys hi tsp tys_shape; replace c3 := c3 i mn R (by grind);
          ...
          apply And.intro
          · { simp [c3a, mk_method_om]; rw[c3a]; simp; sorry }
          · apply And.intro; apply c3b; sorry
      · apply ih wftl h1
  ...
  case _ ih => -- instance
    ...
    · apply Intermediate.GlobalWf.inst
      · assumption
      · apply h4
      · have lem := spine_kinding_SI_transfer (test' := Intermediate.Ty.data? Core.DataConst.opn Γ') wftl h1
                      (by intro T h; apply Ty.data?_SI_transfer wftl h1 T h) q1
        apply lem
      · apply h7
      · intro i hi;
    ...
    case classDecl _ s mτs na Ks1 c1 c2 c3 =>
      ...
      case _  h1 =>
        ...
        · simp at h; rcases h with ⟨e1, e2, e3⟩; subst e1; subst e2; subst e3;
          ...
          let tys := (((List.range na).map (t#·)).reverse)
  ...
  case _ cls1 iname na' Ks1' nb' Ks2' nc' As' R' ts _ ih => -- inst
    ...
    · simp [Option.toTM] at h3
      · split at h3 <;> simp [Except.bind_eq_ok_iff] at h3;
        ...
        have e := translate_SI_lookup_odata wftl h lks lki2
...
:= by
  ...
  case _ ih => -- data
    ...
    apply And.intro; apply translate_IC_spine_kinding wftl h c1 c1'; apply And.intro; apply c2'; apply translate_IC_lookup_none wftl h c3'
    apply c2
    apply translate_IC_lookup_none wftl h c1
    apply ih wftl h
  case _ ih => -- defn
    ...
    · apply translate_IC_lookup_none wftl h lk
    apply ih wftl h
  case _ s k1 Ks fds sds mths _ ih => -- class
    ...
    have lem : ⊢ (Core.Global.odata s (mk_cls_kind Ks) :: Γ') := by
      constructor; constructor; apply translate_IC_lookup_none wftl h c1; apply ih wftl h
    sorry
  case _ iname cls_name k1 k2 k3 Ks1 Ks2 As fds scs mths _ ih => -- inst
    ...
    := by constructor;
          ...
          · apply translate_IC_lookup_none wftl h h4
