...
def ctx : Ctx Term := [
  ...
  -- .term (∀[★](∀[★](∀[★] (#11 `@k #2 `@k #1) -t> (#12 `@k #3 `@k #1) -t> #17)))
  --          (Λ[★]Λ[★]Λ[★]Λ[★] `λ[#3 ~ #2] `λ[#13 `@k #4 `@k #1]
  --            (#0 ▹ (∀c[★]∀c[★](#17 `@k #1 `@k #2) `@c[#1] `@c[refl! #3]))),
  -- inj : ∀ t a a'. (Maybe a ~ t) → (Maybe a' ~ t) → a ~ a'
  -- .term (∀[★]∀[★]∀[★] (#6 `@k #1 ~ #2) -t> (#7 `@k #1 ~ #3) -t> (#3 ~ #2))
  --       (Λ[★]Λ[★]Λ[★] `λ[#6 `@k #1 ~ #2]`λ[#7 `@k #1 ~ #3] (#1 `; sym! #0).!2),
