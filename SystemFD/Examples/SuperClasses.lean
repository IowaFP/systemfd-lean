...
def OrdEqBoolCtx : Ctx Term := [
     ...
     -/
     -- .inst #4 (Λ[★] `λ[#6 `@k #0]
     --   Term.guard (#4 `@t #1) #0 (`λ[#1 ~ #10] (`λ[10]`λ[11]

     --   ))
     -- ),
    -- or : Bool → Bool → Bool
     .term (#12 -t> #13 -t> #14) (`λ[#12] `λ[#13]
           Term.ite #12 #1 #12 (Term.ite #13 #1 #0 #13)),



     -- Λa λ (d: Ord a). OrdBool[a] ← #0 then EqBool[a]
     .inst #1 (Λ[★] `λ[#4 `@k #0]
           Term.guard (#2 `@t #1) #0 (#8 `@t #1)),

     .insttype (∀[★](#0 ~ #11) -t> (#4 `@k #1)), -- OrdBool : ∀ t. Bool ~ t → Ord t
