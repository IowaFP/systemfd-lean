...
abbrev Determined Γ := ∀ x, Γ.is_openm x ->
  ...
  σ.groundTy Γ ->
  (Γ ⊢ ((#x) ⬝[τs]⬝ ds ⬝) : σ) -> -- TODO generalize this to arbitrary spine
  -- (Γ ⊢ ((#x).apply_spine sp) : σ)
  -- (∀ τ ∈ τs, τ.groundTy Γ) ->
  (∀ d ∈ ds, ∃ κ, DeterminedWellTypedVal Γ κ d) ∧
...
theorem subst_determined_stable_lemma {Γ Δ : Ctx Term} {σ : Subst Term} {τ e}:
  (∀ n : Nat, ∃ t, σ n = .su t -> ∃ τ, DeterminedWellTyped Γ τ t) ->
...
intro h1 h2
apply @subst_determined_stable_lemma (Frame.type τ' :: Γ) Γ _ τ t
case _ =>
  intro n
  induction n
  exists e; intro h; exists τ;
  sorry
  sorry
assumption
