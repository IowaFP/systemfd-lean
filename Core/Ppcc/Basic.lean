...
def EqGraph.appc (G : GlobalEnv) (wf : ⊢ G) (Δ : KindEnv) (Γ : TyEnv) (t1 t2 : Term) (KA KB : Kind) (A1 A2 B1 B2 : Ty)
  (j1 : (G&Δ, Γ ⊢ t1 : (A1 ~[KA -:> KB]~ A2))) (j2 : (G&Δ, Γ ⊢ t2 : (B1 ~[KA]~ B2))) :
  ((t : Term) ×' (G&Δ, Γ ⊢ t : ((A1 • B1) ~[KB]~ (A2 • B2)))) :=
  ⟨(((((Term.appc KA KB •[A1]) •[A2]) •[B1]) •[B2]) • t1) • t2,
   by have lem1 := terms_have_star_types wf j1
      have lem2 := terms_have_star_types wf j2
      cases lem1; case _ jA1 jA2 =>
      cases lem2; case _ jB1 jB2 =>
      apply Typing.app (A := B1 ~[KA]~ B2)
      · apply Typing.app (A := (A1 ~[KA -:> KB]~ A2))
        · apply Typing.appt (K := KA)
                (P := (A1⟨Ren.succ Ty⟩ ~[KA -:> KB]~ A2⟨Ren.succ Ty⟩) -:> (B1⟨Ren.succ Ty⟩ ~[KA]~ t#0) -:> ((A1⟨Ren.succ Ty⟩ • B1⟨Ren.succ Ty⟩) ~[KB]~ (A2⟨Ren.succ Ty⟩ • t#0)))
          · apply Typing.appt (K := KA)
                (P := ∀[KA](A1⟨Ren.succ Ty⟩⟨Ren.succ Ty⟩ ~[KA -:> KB]~ A2⟨Ren.succ Ty⟩⟨Ren.succ Ty⟩) -:> (t#1 ~[KA]~ t#0) -:> ((A1⟨Ren.succ Ty⟩⟨Ren.succ Ty⟩ • t#1) ~[KB]~ (A2⟨Ren.succ Ty⟩⟨Ren.succ Ty⟩ • t#0)))
            · apply Typing.appt (K := KA -:> KB)
                (P := ∀[KA]∀[KA](A1⟨Ren.succ Ty⟩⟨Ren.succ Ty⟩⟨Ren.succ Ty⟩ ~[KA -:> KB]~ t#2) -:> (t#1 ~[KA]~ t#0) -:> ((A1⟨Ren.succ Ty⟩⟨Ren.succ Ty⟩⟨Ren.succ Ty⟩ • t#1) ~[KB]~ (t#2 • t#0)))
              · apply Typing.appt (K := KA -:> KB)
                 (P := ∀[KA -:> KB]∀[KA]∀[KA](t#3 ~[KA -:> KB]~ t#2) -:> ((t#1 ~[KA]~ t#0) -:> ((t#3 • t#1) ~[KB]~ (t#2 • t#0))))
                · simp only [Term.appc];
                  apply Typing.lamt
                  · apply Kinding.all; apply Kinding.all; apply Kinding.all; apply Kinding.all; apply Kinding.arrow;
                    · apply Kinding.eq <;> (apply Kinding.var; simp)
                    · apply Kinding.arrow
                      · apply Kinding.eq <;> (apply Kinding.var; simp)
                      · apply Kinding.eq <;> (apply Kinding.app (A := KA); repeat (apply Kinding.var; simp))
                  · apply Typing.lamt
                    · apply Kinding.all; apply Kinding.all; apply Kinding.all; apply Kinding.arrow;
                      · apply Kinding.eq <;> (apply Kinding.var; simp)
                      · apply Kinding.arrow
                    ...
                    · apply Typing.lamt
                      ...
                      · apply Typing.lamt
                        ...
                        · apply Typing.lam
                          ...
                          · apply Typing.lam
                            · apply Kinding.eq <;> (apply Kinding.var; simp)
                            · apply Typing.cast (K := KA -:> KB) (A := t#3) (B := t#2)
                              · apply Kinding.eq <;> (apply Kinding.app (A := KA); repeat (apply Kinding.var; simp))
                              · apply Typing.var
                                · simp
                                · apply Kinding.eq <;> (apply Kinding.var; simp)
                              · apply Typing.cast (K := KA) (A := t#1) (B := t#0)
                                · apply Kinding.eq <;> (apply Kinding.app (A := KA); repeat (apply Kinding.var; simp))
                                · apply Typing.var
                                  · simp
                                  · apply Kinding.eq; repeat (apply Kinding.var; simp)
                                · simp; apply Typing.refl
                                  · apply Kinding.app (A := KA); repeat (apply Kinding.var; simp)
                                · simp
                              · simp
                · apply jA1
                · simp
...
structure EqGraph (G : GlobalEnv) (Δ : KindEnv) (Γ : TyEnv) where
  ...
  /-- number of equiv classes min bound -/
...
def EqGraph.empty {G : GlobalEnv} {Δ : KindEnv} {Γ : TyEnv} : EqGraph G Δ Γ
  := { nodes := [], equiv_class_count := 0, parent_lt := nofun, parent_idx_lt := nofun, parent_kind_eq := nofun
     , unique_nodes := nofun
     /-, equiv_class_count_ub := nofun
     , equiv_class_count_lb := nofun-/ }
...
def EqGraph.push {G : GlobalEnv} {Δ : KindEnv} {Γ : TyEnv}(eG : EqGraph G Δ Γ) (n : Node G Δ) : EqGraph G Δ Γ :=
  ...
  then eG
       ...
       , equiv_class_count := eG.equiv_class_count + 1
       , parent_lt := by
          intro i h1;
          simp;
       ...
       , parent_kind_eq := by
        ...
        , unique_nodes := by
          intro i_idx j_idx hi hj ne

          cases Nat.decLt i_idx eG.nodes.length
          case _ =>
            have lemi : i_idx = eG.nodes.length := by simp at hi; omega
            subst lemi; simp;
            cases Nat.decLt j_idx eG.nodes.length
            case _ =>
              have lemj : j_idx = eG.nodes.length := by simp at hj; omega
              subst lemj; simp; omega
            case _ =>
              rw[List.getElem_append_left (by grind)]; simp [elem_node] at h;
              replace h := h (eG.nodes[j_idx]) (by simp)
              simp [new_node]; intro hi; apply h; symm at hi; apply hi
          case _ =>
            rw[List.getElem_append_left (by grind)]
            cases Nat.decLt j_idx eG.nodes.length
            case _ h =>
              have lemj : j_idx = eG.nodes.length := by simp at hj; omega
              subst lemj; simp; clear h;
              simp [elem_node] at h; replace h := h eG.nodes[i_idx] (by simp);
              intro hi; rw[hi] at h; simp [new_node] at h
            case _ =>
              rw[List.getElem_append_left (by grind)];
              apply eG.unique_nodes
              apply ne
        /-, equiv_class_count_ub := by sorry
        , equiv_class_count_lb := by simp-/
        }
...
def EqGraph.union {G : GlobalEnv} {Δ : KindEnv} {Γ : TyEnv} (wf : ⊢ G) (eG : EqGraph G Δ Γ) (K : Kind)
  ...
  | some n1, some n2 => do
    ...
    match h3 : eG.nodes[ip1]?, h4 : eG.nodes[ip2]? with
    | some p1n, some p2n =>
    if ip1 == ip2
    then none -- both T1 and T2 are already in the same eq class should never happen
      ...
      if h : ip1 < ip2 && (ip2 < eG.nodes.length && (K1 == K2 && (n2.kind == K1 && (n1.kind == K1
         && (K == K1 && (p1n.ty == rep_T1 && (p1n.kind == K1 && (p2n.ty == rep_T2 && (p2n.kind == K2 && rep_T1 != rep_T2)))))))))
      then
        -- pT2 == T2 == T1 == pT1
        -- symm η2 ; (symm t ; η1)
        let new_node : EqGraphNode G Δ Γ := {
          ...
          parent_rel :=
            ...
            let ⟨symm_t_η1, j⟩ := EqGraph.seq G wf Δ Γ symm_t η1 K T2 T1 rep_T1 j'
                         (by simp at h; rcases h with ⟨e1, e2, e3, e4, e5, e6, e7, e8, e9, e10, ne11⟩; subst e5; subst e6; apply j1)
        ...
        some { nodes := eG.nodes.set ip2 new_node
             ...
             , unique_nodes := by
                intro i_idx j_idx hi hj ne
                rw[List.getElem?_eq_some_iff] at h3 h4;
                rcases h3 with ⟨_, h3⟩; rcases h4 with ⟨_, h4⟩
                have ih := @eG.unique_nodes i_idx j_idx (by grind) (by grind) ne
                have lemi := List.getElem_set hi
                have lemj := List.getElem_set hj
                simp at h; rcases h with ⟨e1, e2, e3, e4, e5, e6, e7, e8, e9, e10⟩; subst e5; subst e6; subst e3
                cases Nat.decEq ip2 i_idx
                case _ ni =>
                  cases Nat.decEq ip2 j_idx
                  case _ nj =>
                    rw[ite_cond_eq_false (h := by grind)] at lemi
                    rw[ite_cond_eq_false (h := by grind)] at lemj
                    rw[lemi, lemj]; apply ih
                  case _ nj =>
                    rw[ite_cond_eq_false (h := by grind)] at lemi
                    rw[ite_cond_eq_true (h := by grind)] at lemj
                    subst nj; rw[lemj, lemi]; simp only [new_node, EqGraphNode.ty]; rw[h4] at ih
                    apply ih
                case _ ni =>
                  subst ni;
                  rw[ite_cond_eq_true (h := by grind)] at lemi
                  rw[ite_cond_eq_false (h := by grind)] at lemj
                  rw[lemi, lemj]; simp only [new_node, EqGraphNode.ty]; rw[h4] at ih; apply ih
             /-, equiv_class_count_ub := by
                sorry
             , equiv_class_count_lb := by sorry -/
             }
        ...
        -- sym η1 ; t ; η2
         ...
         some { nodes := eG.nodes.set ip1 new_node
              ...
              , unique_nodes := by
                 ...
                 have lemj := List.getElem_set hj
                 simp at h; rcases h with ⟨e1, e2, e3, e4, e5, e6, e7, e8, e9, e10⟩; subst e5; subst e6; subst e3
                 cases Nat.decEq ip1 i_idx
                 case _ ni =>
                   cases Nat.decEq ip1 j_idx
                   case _ nj =>
                    rw[ite_cond_eq_false (h := by grind)] at lemi
                    rw[ite_cond_eq_false (h := by grind)] at lemj
                    rw[lemi, lemj]; apply ih
                   case _ nj =>
                    rw[ite_cond_eq_false (h := by grind)] at lemi
                    rw[ite_cond_eq_true (h := by grind)] at lemj
                    subst nj; rw[lemj, lemi]; simp only [new_node, EqGraphNode.ty]; rw[h3] at ih
                    apply ih
                 case _ ni =>
                  subst ni;
                  rw[ite_cond_eq_true (h := by grind)] at lemi
                  rw[ite_cond_eq_false (h := by grind)] at lemj
                  rw[lemi, lemj]; simp only [new_node, EqGraphNode.ty]; rw[h3] at ih; apply ih
              /-, equiv_class_count_ub := sorry
              , equiv_class_count_lb := by intro h; sorry -/
              }
        else none
    | _, _ => none
  | _, _ => none
...
| (.app A1 B1), (.app A2 B2), ⟨c, j3⟩ => do
  ...
  let eG' <-
     do match hA1 : A1.infer_kind G Δ, hA2 : A2.infer_kind G Δ, hB1 : B1.infer_kind G Δ, hB2 : B2.infer_kind G Δ with
        ...
        | _, _, _, _ => none
  let ⟨ip1, rep_T1, K1, η1, j1⟩ <- EqGraph.get_rep wf eG' (.app A1 B1) -- Maybe • 3
        ...
        then
          by simp at h; rcases h with ⟨e1, e2, e3, e4, e5, e6⟩;
             ...
             apply do
               ...
               acc.process_equation G wf Δ Γ K n1.ty n2.ty ⟨η1_symm_c1_c_c2_symm_η2, j⟩
        else none)
