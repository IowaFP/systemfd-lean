...
theorem pattern_binders_sound {v : DataConst} {G : GlobalEnv} {Δ : KindEnv} {m : Nat} {Ts : Vec Ty m} {p : Pattern m}:
  ...
  PatternBinders v G Δ m Ts p ζ ξ
:= by
  intro h
  induction m generalizing ξ ζ <;> try simp at *
  case _ =>
  ...
  case _ n ih =>
    ...
    replace h7 := Vec.eq_sound_lem h7; subst h7
...
theorem query_match_sound : query_match q ps = some () -> Query.Match q ps
:= by
  intro h
...
:= by
  ...
  case _ m n ss ps ts smτs ih1 ih2 => -- match
    ...
    · intro i; replace ih2 := @ih2 ζ ξ i (T⟨.add Ty ζ[i].length⟩)
      ...
      replace h8 := h8 i; unfold ζ; unfold ξ; rw[h8]; rw[Vec.to_get_elem]; simp
