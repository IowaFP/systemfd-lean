...
def enumerate_ctor_names {m : Nat} (G : GlobalEnv) (Ss : Vec Ty m) : Option ((n : Nat) × Fun.Vec (Vec String m) n) := do
...
#eval Vec.map (lookup_ctor_names Γ) (#𝓋[ (gt#"Maybe" • gt#"Bool"), gt#"Bool"])
#eval append #𝓋[ "Nothing", "Just"] #𝓋[ "True" , "False" ]
#eval combine #𝓋[ #𝓋["Nothing", "()"], #𝓋["Just" , "()"]] #𝓋[ "True" , "False" , "Med" ]
#eval combine #𝓋[ #𝓋[] ] #𝓋["True", "False"]
#eval combine #𝓋[ #𝓋["Nothing", "()"], #𝓋["Just" , "()"]] #𝓋[ "True" ]

end Test

-- returns a matrix of constructor names from a pattern matrix
@[simp]
def pattern_to_ctor_tags (ps : Fun.Vec (Pattern m) n) : Fun.Vec (Vec String m) n := λ i =>
 Fun.Vec.to (λ j => ((ps i).get_elem j).1)


-- Checks that the patterns are exhaustive
def check_exhaustive (G : GlobalEnv) (Ss : Vec Ty m) (ps : Fun.Vec (Pattern m) n) : Option Unit := do
  let ⟨_, ref_matrix⟩ <- enumerate_ctor_names G Ss

  -- just keep the constructor names from the patterns
  let ps' := pattern_to_ctor_tags ps

  -- check that each entry in ref_matrix has an associated entry ps'
  let mbs := ref_matrix.to.map (λ r => ps'.to.findIdx (λ x => x == r))
  let _ <- mbs.seq
  return ()
