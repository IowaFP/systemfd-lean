...
def MRCtx : GlobalEnv := [
  ...
  .inst "to" #(⟨"FlipPairFun", 2, #(t#2, t#0), 2, 2⟩, ⟨"FlipPairFun", 2, #(t#1, t#0), 2, 2⟩) (
        ...
        let t2 := (prj[1] t1)
        let t3 := (d#"sym").mkApps [t#5, ((gt#"Pair" • t#0) • t#1)] [#2]
        let t4 := (d#"flipPair").mkApps [t#1, t#0] [Term.cast t#0 t2 ((d#"flipPair").mkApps [t#2, t#3] [Term.cast t#0 #4 #0])]
        λ[t#6] Term.cast t#0 t3 t4
  ),
...
#eval! do
  ...
  | some (.openm y ⟨_, Ks1, _, Ks2, n, Ts, R⟩) =>
      if "to" == y then
        ...
        let (ζ, Γ) <- pattern_binders (.data .opn) MRCtx Δ n Ts #(⟨"FlipPairFun", 2, #(t#2, t#0), 2, 2⟩, ⟨"PairPairFun", 2, #(t#1, t#0), 2, 2⟩)
        ...
        let t3 := (d#"sym").mkApps [t#5, ((gt#"Pair" • t#1) • t#0)] [#2]
        let t4 := Term.cast t#0 t2 ((d#"flipPair").mkApps [t#2, t#3] [Term.cast t#0 #4 #0])
        -- λ[t#6] (Term.cast t#0 #4 #0)
        let R' := (λ[t#6] Term.cast t#0 t3 t4).infer_type MRCtx (ζ ++ Δ) Γ
