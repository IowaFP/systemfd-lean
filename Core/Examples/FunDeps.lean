...
def FunDepsCtx : List Global := [
  ...
  .defn "id" (∀[★]∀[★] ((gt#"Equal" • t#1) • t#0) -:> t#1 -:> t#0) (
    Λ[★]Λ[★] λ[((gt#"Equal" • t#1) • t#0) ] λ[t#1]
      (#0 ▹ (((((g#"fdFwd" •[t#1]) •[t#1]) •[t#0]) •(.open) (((g#"EqualTT" •[t#1]) •[t#1]) • (refl! t#1))) •(.open) #1))
  ),
  ...
  --     guard EqualTT[t, u'] <- i0 then λ c2 : t ~ u'.
  --         sym c1 ; c2
  .inst "fdFwd" (Λ[★]Λ[★]Λ[★] λ[(gt#"Equal" • t#2) • t#1] λ[(gt#"Equal" • t#2) • t#0 ]
    .guard ((g#"EqualTT" •[t#2]) •[t#1]) #1 (λ[t#2 ~[★]~ t#1]
      .guard ((g#"EqualTT" •[t#2]) •[t#0]) #1 (λ[t#2 ~[★]~ t#0]
            (sym! #1 `; #0))
    )
  ),
  ...
  .openm "fdBwk" (∀[★] ∀[★] ∀[★] ((gt#"Equal" • t#2) • t#0) -:>  ((gt#"Equal" • t#1) • t#0) -:> (t#2 ~[★]~ t#1)),
