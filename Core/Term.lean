...
notation "fst[" K "]" A => Term.ctor1 (Ctor1Variant.fst K) A
notation "snd[" K "]" A => Term.ctor1 (Ctor1Variant.snd K) A
...
@[simp]
theorem Ctor1Variant.subst_fst : (fst K)[σ:Ty] = fst K := by
  simp [Subst.apply, SubstMap.smap]

@[simp]
theorem Ctor1Variant.subst_snd : (snd K)[σ:Ty] = snd K := by
  simp [Subst.apply, SubstMap.smap]
...
theorem Term.subst_match
  : (match! t1 t2 t3)[σ:Term] = match! t1[σ:_] t2[σ:_] (λ i => (t3 i)[σ:_])
:= by
  simp [Subst.apply, SubstMap.smap]
...
theorem Term.Ty.subst_match
  : (match! t1 t2 t3)[σ:Ty] = match! t1[σ:_] t2[σ:_] (λ i => (t3 i)[σ:_])
:= by
  simp [Subst.apply, SubstMap.smap]
