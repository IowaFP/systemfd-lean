...
instance instRenMap_SurfaceTy : RenMap Ty Ty where
...
@[simp]
theorem Ty.ren_then {A B : Ty} {r : Ren Ty} : (A `=:> B)⟨r⟩ = A⟨r⟩ `=:> B⟨r⟩ := by
  simp [RenMap.rmap]
...
instance instSubstMap_TyTy : SubstMap Surface.Ty Surface.Ty where
...
@[simp]
theorem Ty.subst_then {σ : Subst Ty} {A B : Ty} : (A `=:> B)[σ] = A[σ] `=:> B[σ] := by
  simp [SubstMap.smap]
...
instance instSubstMapId_TyTy: SubstMapId Surface.Ty Surface.Ty where
  apply_id := by subst_solve_id
