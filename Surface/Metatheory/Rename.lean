...
import Surface.Metatheory.FreeVars
...
theorem Kinding.closed : G&[] ⊢s A : K -> ∀ σ, A[σ] = A := by
  intro j; apply closed_rep j

-- theorem ValidInstTy.closed : ValidInstTy G x Δ T -> ∃ b, G&Δ ⊢s T : .base b := by
...
theorem GlobalWf.head {G : List Global} : ⊢s (g :: G) -> GlobalWf G g := by
  intro j; cases j; case _ j => exact j

theorem GlobalWf.tail {G : List Global} : ⊢s (g :: G) -> ⊢s G := by
...
case arrow A B _ _ _ ih1 ih2 =>
  ...
  · intro i h1
    ...
    revert i; apply h
case «then» A B _ _ _ ih1 ih2 =>
  apply Kinding.then
  apply ih1
  · intro i h
    replace h : i ∈ (A `=:> B) := by apply Ty.FV.thenr; assumption
    revert i; apply h
  apply ih2
  · intro i h1
    replace h1 : i ∈ (A `=:> B) := by apply Ty.FV.thenl; assumption
    revert i; apply h
...
theorem Kinding.rename_lift {Δ Δr : KindEnv} K (r : Ren) :
  (∀ i, Δ[i]? = Δr[r i]?) ->
...
theorem Kinding.weaken T : G&Δ ⊢s A : K -> G&(T::Δ) ⊢s A[+1] : K := by
  intro j; apply rename (T::Δ) (· + 1) _ j
  intro i; cases i <;> simp [KindEnv, inst_getElem?_KindEnv] at *
...
:= by
  intro wf h j; sorry
  -- match j with
  -- | var j1 j2 => simp [Ren.to]; apply Typing.var (h j1) j2
  -- | global j1 j2 => apply Typing.global j1 j2
  -- | mtch PTy pats cs ih1 ih2 ih3 ih4 ih5 ih6 ih7 ih8 ih9 => sorry
...
def Typing.weaken T : ⊢s G -> G&Δ,Γ ⊢s t : A -> G&Δ,(T::Γ) ⊢s t[+1] : A := by
  intro wf j; apply rename (T::Γ) (· + 1) wf _ j; simp
  intro i A h
  simp [TyEnv, inst_getElem?_TyEnv] at *; assumption

-- theorem Kinding.closed_lifting_lemma : ∀ aΔ' : KindEnv, ⊢s G ->  G&Δ ⊢s T : K ->
--         (G&(Δ' ++ Δ) ⊢s T[Ren.to (λ x => (x + Δ'.length))] : K) := by
-- intro Δ' wf j
-- apply @List.reverse_ind (T := Kind)
--   (motive := λ Δ' => ∀ G Δ T K,  ⊢s G -> G&Δ ⊢ T : K -> (G&(Δ' ++ Δ) ⊢ T[Ren.to (λ x => (x + Δ'.length))] : K))
--   Δ'
