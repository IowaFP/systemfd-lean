...
def TrueFalsePat : Core.Pattern 2 := #(⟨"False", 0 , #() , 0, 0⟩, ⟨"True", 0 , #() , 0, 0⟩)
def FalseTruePat : Core.Pattern 2 := #(⟨"True", 0 , #() , 0, 0⟩, ⟨"False", 0 , #() , 0, 0⟩)
def TrueCtor :  Term := g`#"True" `•ᵤ #() `•ₑ #() `•ₜ .nil
def FalseCtor :  Term := g`#"False" `•ᵤ #() `•ₑ #() `•ₜ .nil

def LTCtor :  Term := g`#"LT" `•ᵤ #() `•ₑ #() `•ₜ .nil
def GTCtor :  Term := g`#"GT" `•ᵤ #() `•ₑ #() `•ₜ .nil
def EQCtor :  Term := g`#"EQ" `•ᵤ #() `•ₑ #() `•ₜ .nil


def benv : GlobalEnv := [
  .defn "test" (gt#"Bool" -:> gt#"Bool") ((g`#"eq" `•ᵤ #(gt#"Bool") `•ₑ #() `•ₜ .nil) `• TrueCtor :: gt#"Bool"),
