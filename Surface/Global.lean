...
| classDecl s Ks fds methods =>
  ".class " ++ s ++ " : " ++ Ks.repr max_prec ++ "|" ++ fds.repr max_prec
...
def is_opent (G : GlobalEnv) x := lookup x G |> Option.map Entry.is_opent |> Option.get!
