...
def hs_lift_subst_replace (A : Frame HsTerm) :
  ⊢s (A.apply σ :: Δ) ->
  (∀ v n t T, σ n = .su t -> .some T = (Γ d@ n).get_type -> HsJudgment v Δ (hs_idx_subst_ty σ (t, T))) ->
  (∀ v n t T, ^σ n = .su t -> .some T = ((A::Γ) d@ n).get_type -> HsJudgment v (A.apply σ :: Δ) (hs_idx_subst_ty (^σ) (t, T)))
:= by
intro j h1 v n t T h2 h3
cases n <;> simp at *
case _ n =>
  ...
  case _ t' =>
    ...
    simp at lem
    replace h1 := h1 v n t' ([P]T) ydef lem
...
| Γ, Δ, σ, .ctx, (), f1, f2, f3,j, wf => wf
| Γ, Δ, σ, .kind, (t, τ), f1, f2, f3, j, wf => match j with
  | .ax wf' => by constructor; assumption
  | @HsJudgment.arrowk Γ A B h1 h2 => by
    have lem1 := @hs_subst Γ Δ σ .kind _ f1 f2 f3 h1 wf; simp at lem1
    have lem2 := @hs_subst Γ Δ σ .kind _ f1 f2 f3 h2 wf; simp at lem2
    constructor; assumption; assumption;

| Γ, Δ, σ, .type, (t, τ), f1, f2, f3, j, wf => match j with
  | @HsJudgment.varTy Γ x T h1 h2 h3 h4 => by
    ...
    case _ y =>
      ...
      have lem2 := f2 .type x
      have lem3 := @hs_subst Γ Δ σ .kind (T, `□) f1 f2 f3 h4 wf
      rw[zdef]; simp;
      ...
      apply lem3
    case _ a =>
      have lem := f2 .type x a T zdef h3
      rw[zdef]; simp;
      apply lem

  | .appk h1 h2 h3 h4 => by
     have lem1 := @hs_subst Γ Δ σ .type _ f1 f2 f3 h1 wf
     have lem2 := @hs_subst Γ Δ σ .type _ f1 f2 f3 h2 wf
     have lem3 := @hs_subst Γ Δ σ .kind _ f1 f2 f3 h3 wf
     have lem4 := @hs_subst Γ Δ σ .kind _ f1 f2 f3 h4 wf
  ...
  | @HsJudgment.farrow Γ A B h1 h2 h3 => by
    ...
    have lem3 : (Frame.empty :: Δ) ⊢τ ([^σ] B) : `★ := by
      apply @hs_subst (.empty :: Γ) (.empty :: Δ) (^σ) .type _ f1' f2' f3' h3 wf'
  ...
  | @HsJudgment.allt Γ A B h1 h2 => by
    have lem1 := @hs_subst Γ Δ σ .kind _ f1 f2 f3 h1 wf
    ...
    have lem2 : (Frame.kind ([σ]A) :: Δ) ⊢τ ([^σ]B) : `★ := by
      apply hs_subst .type f1' f2' f3' h2 wf'
    constructor; assumption; assumption
...
| Γ, Δ, σ, .term, (t, τ), f1, f2, f3, j, wf => match j with
  | @HsJudgment.implicitAllI Γ A t τ h1 h2 => by
    simp
    have lem2 := @hs_subst Γ Δ σ .kind _ f1 f2 f3 h2 wf
    have wf' : ⊢s (.kind ([σ]A) :: Δ) := by
      constructor; assumption; assumption
    have f1' := hs_lift_subst_rename (.kind A) f1
    have f2' := hs_lift_subst_replace (.kind A) wf' f2
    have f3' := hs_lift_subst_stable (.kind A) f3
    have lem := @hs_subst (.kind A :: Γ) (.kind ([σ]A) :: Δ) (^σ) .term _ f1' f2' f3' h1 wf'; simp at lem
  ...
  | @HsJudgment.implicitArrI Γ π τ t h1 h2 h3 h4 => by
    ...
    have wf' : ⊢s (.empty :: Δ) := by
      constructor; assumption
    have lem1 : Δ ⊢τ ([σ]π) : `★ := @hs_subst Γ Δ σ .type _ f1 f2 f3 h1 wf
    have f1' := hs_lift_subst_rename .empty f1
    have f2' := hs_lift_subst_replace .empty wf' f2
    have f3' := hs_lift_subst_stable .empty f3
    have lem2 : (.empty :: Δ) ⊢τ ([^σ]τ) : `★ := by
      apply @hs_subst (.empty :: Γ) (.empty :: Δ) (^σ) .type _ f1' f2' f3' h2 wf'
    have wf'' : ⊢s (.type ([σ]π) :: Δ) := by
      constructor; assumption; assumption
    have f1'' := hs_lift_subst_rename (.type π) f1
    have f2'' := hs_lift_subst_replace (.type π) wf'' f2
    have f3'' := hs_lift_subst_stable (.type π) f3
    have lem3 : (.type ([σ]π) :: Δ) ⊢t ([^σ][S]t) : ([^σ]τ) := by
      apply @hs_subst (.type π :: Γ) (.type ([σ]π) :: Δ) (^σ) .term _ f1'' f2'' f3'' h4 wf''
  ...
  | @HsJudgment.implicitAllE Γ A τ t e τ' h1 h2 h3 h4 h5 => by
    ...
    apply @hs_subst Γ Δ σ .type _ f1 f2 f3 h1 wf
    apply @hs_subst Γ Δ σ .kind _ f1 f2 f3 h2 wf
    apply @hs_subst Γ Δ σ .term _ f1 f2 f3 h3 wf
    apply @hs_subst Γ Δ σ .type _ f1 f2 f3 h4 wf
  ...
  | @HsJudgment.implicitArrE Γ t π τ e τ' h1 h2 h3 h4 => by
    ...
    apply @hs_subst Γ Δ σ .term _ f1 f2 f3 h1 wf
    apply @hs_subst Γ Δ σ .term _ f1 f2 f3 h2 wf
  ...
  | @HsJudgment.lam Γ A t B h1 h2 h3 => by
    ...
    have f3' := hs_lift_subst_stable (.type A) f3
    ...
    assumption
    apply @hs_subst (.type A :: Γ) (.type ([σ]A) :: Δ) (^σ) .term _ f1' f2' f3' h2 wf'
    apply @hs_subst (.empty :: Γ) (.empty :: Δ) (^σ) .type _ f1'' f2'' f3'' h3 wf''
  ...
  | @HsJudgment.app Γ t1 A B t2 B' h1 h2 h3 h4 h5 => by
    ...
    rw[h3]; rw[Subst.apply_compose_commute]; simp; rw[<-Subst.lift_unfold]; rw[subst_valid]; rfl
    apply (@hs_subst Γ Δ σ .type _ f1 f2 f3 h4 wf)
    apply (@hs_subst Γ Δ σ .type _ f1 f2 f3 h5 wf)


  | .hsIte h1 h2 h3 h4 h5 h6 h7 h8 h9 h10 h11 h12 => by
    apply HsJudgment.hsIte
    apply (@hs_subst Γ Δ σ .type _ f1 f2 f3 h1 wf)
    apply (@hs_subst Γ Δ σ .type _ f1 f2 f3 h2 wf)
    apply (@hs_subst Γ Δ σ .type _ f1 f2 f3 h3 wf)
    apply (hs_subst .type f1 f2 f3 h4 wf)
    apply (hs_subst .term f1 f2 f3 h5 wf)
    apply (hs_subst .term f1 f2 f3 h6 wf)
    apply (hs_subst .term f1 f2 f3 h7 wf)
    apply (hs_subst .term f1 f2 f3 h8 wf)
    apply hs_stable_type_match_subst f1 f3 h9
    apply hs_prefix_type_match_subst f1 f3 h10
  ...
  | @HsJudgment.hslet Γ A t1 B' B t2 h1 h2 h3 h4 h5 => by
    have wf' : ⊢s (.term ([σ]A) ([σ]t1) :: Δ) := by
      ...
      apply hs_subst .type f1 f2 f3 h1 wf
      apply hs_subst .term f1 f2 f3 h2 wf
      apply wf

    have f1' := hs_lift_subst_rename (.term A t1) f1
    have f2' := hs_lift_subst_replace (.term A t1) wf' f2
    have f3' := hs_lift_subst_stable (.term A t1) f3
    ...
    case _ =>
      ...
      have h : (.term ([σ]A) ([σ]t1) :: Δ) ⊢t ([^σ]t2) : ([^σ][S] B) := by
        apply @hs_subst (.term A t1 :: Γ) (.term ([σ]A) ([σ]t1) :: Δ) ^σ .term _ f1' f2' f3' h4 wf'
      apply h
    apply hs_subst .type f1 f2 f3 h5 wf
termination_by _ _ _ _ _ _ _ _ h => h.size
...
cases wf; case _ wf =>
apply @hs_subst (.empty :: Γ) Γ (.su t :: I) .type (τ, k) _ _ _ j wf;
case _ =>
  intro n y h;
...
apply @hs_subst (.kind A :: Γ) Γ (.su t :: I) .type (τ, k) _ _ _ j1 wf;
case _ =>
  intro n y h1
  cases n <;> simp at *; subst h1
  case _ n =>
